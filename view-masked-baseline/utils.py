...
matplotlib.use('agg')
from ReIDdatasets import MSMT_source_train, MSMT_gallery, MSMT_probe_same_view
...
def get_reid_dataloaders(dataset, img_size, crop_size, padding, batch_size, view_labeled, root='/home/zunzhi/reid-view-invariant/data/{}.mat'):
    ...
    train_data = MSMT_source_train(root.format(dataset), view_labeled=view_labeled)
    gallery_data = MSMT_gallery(root.format(dataset))
    probe_data = MSMT_probe_same_view(root.format(dataset), view_labeled=view_labeled)
