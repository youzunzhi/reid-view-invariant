...
def main():
    ...
    train_view_loader, train_label_loader, gallery_loader, probe_loader = get_reid_dataloaders(args.dataset, args.img_size, args.crop_size,
                                                                      args.padding, args.batch_size, args.view_labeled)
    num_classes = train_label_loader.dataset.return_num_class()
    # num_views = train_loader.dataset.return_num_views()
    num_views = 15

    if args.resume:
        trainer, start_epoch = load_checkpoint(args, logger)
    else:
        trainer = ReverseLayerTrainer(args, num_classes, num_views, logger)
        start_epoch = 0
    ...
    for epoch in range(start_epoch, total_epoch):
        ...
        meters_trn = trainer.train_epoch(train_view_loader, train_label_loader, epoch, total_epoch)
