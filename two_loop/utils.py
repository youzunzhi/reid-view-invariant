...
matplotlib.use('agg')
from ReIDdatasets import MSMT_train_view, MSMT_train_label, MSMT_gallery, MSMT_probe
...
def get_reid_dataloaders(dataset, img_size, crop_size, padding, batch_size, view_labeled, root='/home/zunzhi/reid-view-invariant/data/{}.mat'):
    ...
    train_view = MSMT_train_view(root.format(dataset))
    train_label = MSMT_train_label(root.format(dataset), view_labeled=view_labeled)
    gallery_data = MSMT_gallery(root.format(dataset))
    probe_data = MSMT_probe(root.format(dataset), view_labeled=view_labeled)

    mean = train_view.return_mean() / 255.0
    std = train_view.return_std() / 255.0
    ...
    train_view.turn_on_transform(transform=train_transform)
    train_label.turn_on_transform(transform=train_transform)
    gallery_data.turn_on_transform(transform=test_transform)
    probe_data.turn_on_transform(transform=test_transform)


    train_view_loader = torch.utils.data.DataLoader(train_view, batch_size=batch_size, shuffle=True,
                                               num_workers=10, pin_memory=True, drop_last=True)
    train_label_loader = torch.utils.data.DataLoader(train_label, batch_size=batch_size, shuffle=True,
                                               num_workers=10, pin_memory=True, drop_last=True)
    gallery_loader = torch.utils.data.DataLoader(gallery_data, batch_size=batch_size, shuffle=False,
                                                 num_workers=10, pin_memory=True)
    probe_loader = torch.utils.data.DataLoader(probe_data, batch_size=batch_size, shuffle=False,
    ...
    return train_view_loader, train_label_loader, gallery_loader, probe_loader
