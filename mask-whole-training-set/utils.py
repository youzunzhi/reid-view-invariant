...
matplotlib.use('agg')
from ReIDdatasets import MSMT_view_masked
...
def get_reid_dataloaders(dataset, img_size, crop_size, padding, batch_size, view_labeled, root='/home/zunzhi/reid-view-invariant/data/{}.mat'):
    ...
    train_data = MSMT_view_masked(root.format(dataset), state='train', view_labeled=view_labeled)
    gallery_data = MSMT_view_masked(root.format(dataset), state='gallery', view_labeled=view_labeled)
    probe_data = MSMT_view_masked(root.format(dataset), state='probe', view_labeled=view_labeled)
